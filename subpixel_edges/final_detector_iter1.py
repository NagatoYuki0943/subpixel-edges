...
def main_iter1(image: np.ndarray, threshold: int | float, order: int):
    # initialization
    ep = EdgePixel()
    ...
    # all shape: [24]
    h_edges_, h_pixel_x, h_pixel_y, h_x, h_y, h_nx, h_ny, h_curv, h_i0, h_i1 = h_edges(
        image,
        image_flatten,
    ...
    # all shape: [28]
    v_edges_, v_pixel_x, v_pixel_y, v_x, v_y, v_nx, v_ny, v_curv, v_i0, v_i1 = v_edges(
        image,
        image_flatten,
        image_smooth_flatten,
        Gx,
